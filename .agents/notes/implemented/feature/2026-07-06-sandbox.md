...
Denied file effects return a `[sandbox: file access denied under <mode> mode]` marker and instructions not to work around the denial. A confining executor adds paired `sandbox_permissions` and `justification` fields: a matching target reuses the session's effective mode without approval, while a strictly wider target requests one approval for that retry. A grant widens only that retry; rejection executes nothing, returns `the user rejected escalating this command to "<mode>"`, and permits no re-ask. The owner-derived pending policy context states the current file policy without replacing those enforcement boundaries. When `dsh-permission-presets` is composed with a UI adapter, one preset selects both knob values; unmatched values fold to `custom`. The [ACP automation composition](../../../../examples/acp-agent/README.md) does not mount that UI service and selects its deployment mode explicitly.
...
- The escalation fields exist exactly when the mounted executor confines; a target matching the call's effective mode reuses it without approval, a narrower target fails closed with its own text and prompts no one, and a deployment with no ApprovalService fails strictly wider calls closed while leaving plain and matching calls untouched.
...
- **The advertised target set is static while the effective mode is per-session** (schemas are registry-global) — a session already at the widest mode is still offered the fields. Harmless by construction: execution treats a matching target as already satisfied, asks only for a strictly wider target, and rejects a narrower target without prompting anyone.
