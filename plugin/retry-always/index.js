...
export const Config = {
  '~standard': {
    version: 1,
    vendor: '@deepseek-ai/dsh-retry-always',
    validate(value) {
      return value === undefined || isRecord(value)
        ? { value: value ?? {} }
        : { issues: [{ message: 'retry-always config must be an object' }] }
    },
  },
}
...
export function apply(ctx) {
  ...
  if (current === undefined) {
    throw new Error('retry-always requires the llm-pi-ai settings namespace')
  }
  ...
  const reconcile = section => {
    ...
    void ctx.settings.mutate(SETTINGS_NAMESPACE, ops).catch(error => {
      ctx.logger.warn('retry-always could not persist provider policies: %o', error)
    }).finally(() => {
      state.pending = false
  ...
  ctx.effect(() => () => {
    state.disposed = true
    disposeListener()
  }, 'retry-always: dispose settings listener')
