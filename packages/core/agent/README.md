...
- `agent.resumeInterruptedTurn()` — require an idle agent whose final logical event is an interrupted turn, then start a new turn from durable history without appending an ordinary user message.
