...
- id: llm
  ...
  config:
    # Profiles that omit retryPolicy inherit this composition default.
    defaultRetryPolicy:
      mode: always
    providers:
...
`defaultRetryPolicy` applies to profiles that omit a route-level `retryPolicy`; an explicit profile value wins. The shipped `dsh-base` bundle sets this composition default to `always` for Web and Headless profiles, while a standalone composition that omits it keeps the bounded normal default.
