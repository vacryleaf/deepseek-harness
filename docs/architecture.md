...
`resumeInterruptedTurn()` is the no-inbox exception: an idle Agent with an interrupted final logical turn opens a new durable turn from its existing history, so the recovery request adds no ordinary user message.
